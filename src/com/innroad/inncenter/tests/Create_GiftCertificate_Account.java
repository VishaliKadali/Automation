...
public class Create_GiftCertificate_Account extends TestCore {
	...
	public void create_GiftCertificate_Account(String url, String ClientCode, String Username, String Password,
			String propertyName, String Accounttype, String GiftCertificateName, String Amount, String MarketSegment,
			String Referral, String Phonenumber, String alternativenumber, String Email, String city, String State,
		...
		ExtentTest test = extent.startTest("Create_GiftCertificate_Account", "Creating GiftCertificate Account")
		...
		try {
			...
			test.log(LogStatus.FAIL, "Failed to navigate new account page \n" + e.getMessage() + 
					"\n\n <br> Attaching screenshot below : \n" + test.addScreenCapture(Utility.captureScreenShot(testName + "_NewGCAccount_" + Utility.getTimeStamp(), driver)));
		...
		try {
			Account CreateCorporteAccount = new Account();
			CreateCorporteAccount.GiftCertificate(driver, GiftCertificateName);
			CreateCorporteAccount.navigateFolio(driver);
			CreateCorporteAccount.addLineitem(driver, PropertyName, Accounttype, Amount);
			...
			test.log(LogStatus.FAIL, "Failed to save the account \n" + e.getMessage() + 
					"\n\n <br> Attaching screenshot below : \n" + test.addScreenCapture(Utility.captureScreenShot(testName + "_SaveGCAccount_" + Utility.getTimeStamp(), driver)));
...
//			Reservation res = new Reservation();
		...
		try {
			...
			Accountpay.GiftCertificate(driver, Accounttype, ChangeAmount, ChangeAmountValue);
//			Reservation res = new Reservation();
			res.saveReservation(driver);
			app_logs.info("Entered all reqiured details and save the reservation");
...
//		return test data from the sheet-name provided
		return Utility.getData("Create_GiftCertificate_Account", excel);
	}
