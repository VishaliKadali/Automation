...
public class Create_Unitowner_Account extends TestCore {
	...
	public void Create_Unitowner_account(String url, String ClientCode, String Username, String Password,
		...
		test = extent
				.startTest("Create_Unitowner_Account",
						"Create a Unit Owners Account ,make a reservation and verify payments")
		...
		// New account
		try {
			CreateUO.NewAccountbutton(driver, Accounttype);
			CreateUO.AccountDetails(driver, Accounttype, AccountName);
			CreateUO.AccountAttributes(driver, test, MargetSegment, Referral);
			CreateUO.Mailinginfo(driver, test, FirstName, LastName, Phonenumber, alternativeNumber, Address1, Address2,
					Address3, Email, city, State, Postalcode);
			CreateUO.Billinginfo(driver, test);
			CreateUO.AccountSave(driver, test, AccountName);
		...
		try {
			CreateUO.NewReservationButton(driver, test);
		...
		// Payment
		try {
			Card_Payment.CardPayment(driver, PaymentType, Amount, CardName, CCNumber, CCExpiry, CCVCode, Address1, city,
					State, Postalcode, Authorizationtype);
			res.SaveReservation(driver);
