...
public class Reservation_CashPayment extends TestCore {
	...
	public void Reservation_cashPayment(String url, String ClientCode, String Username, String Password,
		...
		ExtentTest test = extent
				.startTest("Reservation_CashPayment", "Creating reservation with payment method as cash")
		...
		try {
			...
			Wait.wait15Second();
			res.saveReservation(driver);
			app_logs.info("Created Reservation with cash Success");
			test.log(LogStatus.PASS, "Created Reservation with cash Success");
		} catch (Exception e) {
			test.log(LogStatus.FAIL, "Failed to Create Reservation \n" + e.getMessage() + 
					"\n\n <br> Attaching screenshot below : \n" + test.addScreenCapture(Utility.captureScreenShot(testName + "_CashPayAndSaveReservation_" + Utility.getTimeStamp(), driver)));
