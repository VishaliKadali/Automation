	...
	public void create_UnitOwner_Account(String url, String ClientCode, String Username, String Password,
		...
		try {
			...
			test.log(LogStatus.FAIL, "Failed to navigate new account page \n" + e.getMessage() + 
					"\n\n <br> Attaching screenshot below : \n" + test.addScreenCapture(Utility.captureScreenShot(testName + "_NewUOAccount_" + Utility.getTimeStamp(), driver)));
		...
		try {
			...
			test.log(LogStatus.FAIL, "Failed to save the account \n" + e.getMessage() + 
					"\n\n <br> Attaching screenshot below : \n" + test.addScreenCapture(Utility.captureScreenShot(testName + "_SaveUOAccount_" + Utility.getTimeStamp(), driver)));
...
//			Reservation res = new Reservation();
			res.saveReservation(driver);
			app_logs.info("paymnet with account and Save reservation success");
